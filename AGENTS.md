...
- Canonical output: `pif.json` on `main`.
...
- Additional checks: `bash scripts/validate-pixel-data.sh pif.json`,
...
- A validated changed crawl commits only `pif.json` directly to `main`.
